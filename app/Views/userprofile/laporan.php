...
?>
    ...
    <script src="/assets/buttons.print.js"></script>
        ...
        <div class="carousel slide" id="ueOkfUJH6x" data-interval="5000" data-bs-interval="5000">
            <div id="carouselExampleIndicators" class="carousel slide" data-ride="carousel">
                <div class="carousel-inner">
                    ...
                    if ($id_user) {
                        $db = \Config\Database::connect();
                        $query = $db->query("SELECT id, sampul, nama_masjid, deskripsi, alamat_masjid, gambar1, gambar2, gambar3 FROM db_data_masjid WHERE id_user = ?", [$id_user]);
...
if ($id_user) {
    $db = \Config\Database::connect();
    $query = $db->query("SELECT id, sampul, nama_masjid, deskripsi, alamat_masjid FROM db_data_masjid WHERE id_user = ?", [$id_user]);
    $result = $query->getRowArray();
    if ($result) {
        $masjid = $result;
    }
                ...
                $('#zakat-table').DataTable({
                    ...
                    layout: {
                        bottomEnd: {
                            buttons: [{
                                extend: 'print',
                                title: '<h3><?= $nama_masjid ?></h3><br/><p><?= $masjid['alamat_masjid'] ?></p></br><center><h2>Laporan Zakat</h2></center>',
